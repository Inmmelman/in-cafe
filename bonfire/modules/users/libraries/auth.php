...
class Auth
{
	...
	public function __construct()
	{
		...
		$this->ci->load->model('users/social_user_model');
    ...
    public function checkUser ($login){
        ...
        $selects = 'id, social_id';
        $commonUser = $this->ci->user_model->select($selects)->find_common('social_id', $login);
        if($commonUser  == FALSE){
            return false;
        }else{
            $user = $this->ci->user_model->select('*')->find($commonUser->id);
        }
        return $user;
    }
	...
	public function user()
	{
		...
		if ($this->ci->session->userdata('identity') && $this->ci->session->userdata('user_id'))
		{
            ...
            //TODO проверка на тип пользвоателя

            $user->social  = $this->ci->social_user_model->find($this->ci->session->userdata('user_id'));
	 ...
	 */
	public  function setup_session($user_id, $username, $password_hash, $email, $role_id, $remember=FALSE, $old_token=NULL,$user_name='')
	{
