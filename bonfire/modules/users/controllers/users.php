...
class Users extends Front_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('users/shebang_user_model');

		$this->load->library('users/auth');

        $this->load->config('fenix');

        if ( ! class_exists('SocialAuther'))
        {
            include APPPATH.'/libraries/SocialAuther/SocialAuther.php';
        }
	...
	public function register()
	{
		...
		if (isset($_POST['register']))
		{
            if(isset($_POST['userinfos'])){
                $this->form_validation->set_rules('phone', 'lang:bf_phone', 'required|integer|min_length[10]|max_length[12]|unique[users.phone]');
            }else{
			...
			if ($this->form_validation->run() !== FALSE)
			{
				...
				if ($user_id = $this->user_model->insert($data))
				{
                    ...
                    if(isset($_POST['userinfos'])){
                        ...
                        $dataForShebangUser = array(
                            'id' => $user_id,
                            'title' =>  $this->input->post('display_name'),
                            'phone' => $this->input->post('phone'),
                        );

                        $dataForShebangUser['id'] = $user_id;
                        $this->shebang_user_model->insert($dataForShebangUser);
                        ...
                        $dataForSocialUser = array(
                            ...
                            'social_page' => '',
                            'sex' => 'none',
                            //'avatar' => "http://www.gravatar.com/avatar/" . md5( strtolower( trim( $this->input->post('email') ) ) ) . "?d=" . urlencode( $this->default_avatar ) . "&s=" . $this->avatar_size
                            'avatar' => "http://www.gravatar.com/avatar/" . md5( strtolower( trim( $this->input->post('email') ) ) ) . "?s=96&r=pg&d=identicon"
                        );


                        $dataForSocialUser['id'] = $user_id;
